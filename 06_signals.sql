...
--
-- ── 2026-09-16 추세추종 3단계 진입조건 조정 (실거래 99건 실증 분석) ──────────
--    그동안의 개선은 전부 "출구(손절·트레일링)" 쪽이었고 진입 조건은 손대지
--    않았다. VIRTUAL TREND 청산 94건을 진입일 V4_CAND_TREND_3 신호의 reason
--    jsonb와 조인해, 어떤 진입 변수가 성패를 가르는지 구간별로 봤다.
--    (기준: 전체 94건 평균 -3.01% · 승률 33% · CRASH_STOP_10 비중 31%)
--
--    거래량비(vol_ratio20_prev) 구간별
--        <80  : 22건 -3.45%  승률 32%      105~120: 11건 -1.61%  승률 45%
--      80~95  : 18건 -4.68%  승률 22%      120~140: 14건 +0.34%  승률 57%
--      95~105 :  9건 -4.42%  승률 22%      140~250: 20건 -3.49%  승률 25%
--    등락률 구간별
--       1~2: 5건 -4.65%   3~4: 7건 -4.24%   4~6: 17건 +0.22%(승률 59%)
--       2~3:12건 -3.06%                     6~8: 16건 -0.27%
--       8~12: 24건 -4.50%  승률 25%  CRASH 46%   ← 가장 나쁜 구간
--
--    **월별 안정성 검증을 통과한 것만 채택했다** (한 달 효과로 생긴 착시 배제):
--      · 거래량비 >=105% — 06/07/08월 3개월 모두 통과군이 탈락군보다 우위
--            (-1.8 vs -2.8 / -5.8 vs -9.3 / -1.0 vs -2.3)     → 채택
--      · 등락률 <8%     — 3개월 모두 우위
--            (-2.2 vs -3.1 / -7.3 vs -12.8 / 0.0 vs -5.5)     → 채택
--      · 거래량비 상한 140% — 전체로는 좋아 보였으나 6·7월엔 뒤집힘
--            (-2.6 vs -1.6 / -8.4 vs -5.5). 8월 효과였음        → 기각
--      · 개별RS 상위 20% — 6·7월 뒤집히고 8월 차이 미미          → 기각
--
--    두 조건 동시 적용 시(실거래 소급): 94건 → 29건, 평균 -3.01% → -0.89%,
--    승률 33% → 45%, CRASH 비중 31% → 17%. 이 기간은 추세추종에 불리한
--    구간이라 어떤 필터로도 흑자가 되지는 않는다 — 손실 폭을 줄이는 조정이다.
--
--    ⚠ 한계: 표본 94건 · 3.5개월. 1·2단계는 관찰용으로 그대로 두고 매수 단계인
--    3단계에만 적용했으므로, 되돌리려면 3단계의 두 줄만 지우면 된다.
--
...
SELECT trade_date, code, 'V4_CAND_TREND_3', 'WATCH', score,
  ...
  name || ' 추세추종 3단계(매수) · 개별RS ' || rs20_rank || '위/전체 ' || rs20_universe_n || '종목'
       ...
       || ' · 거래량비 ' || round(vol_ratio20_prev) || '%(하한 105%)'
       || ' · 정배열'
       || ' · 종가위치 상위 ' || round(100 - close_pos_pct) || '%'
       || ' · 등락률 ' || round(change_pct, 1) || '%'
FROM scored
WHERE close_pos_pct >= 85 AND change_pct >= 1
  -- 2026-09-16: 3단계(가상매수) 한정으로 등락률 상한 12%→8%.
  AND change_pct < 8
  AND pick_rank_trend <= 40                                        -- 2026-09-11: TREND 1단계 전용 캡 재도입
  AND (near_high OR pct_from_high >= -15)   -- 2026-09-10 -10→-20 완화 후 2026-09-11 -20→-15 부분 복원
  AND ma5 > ma10 AND ma10 > ma20 AND ma20 > ma60
  -- 2026-09-16: 거래량비 **하한 105%** 신설. 그동안 상한(250%)만 있어서 "정배열이고
  -- 신고가 근처인데 거래량은 20일 평균에도 못 미치는" 종목이 그대로 매수됐다.
